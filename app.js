...
function updateHistoryUI() {
  ...
  historyData.forEach((record) => {
    ...
    li.textContent = `${new Date(
      record.timestamp
    ).toLocaleTimeString()} – IP: ${record.ip}, Type: ${record.networkType}`;
...
function saveHistoryRecord(ip, networkType) {
  ...
  if (!lastRecord || lastRecord.ip !== ip || lastRecord.networkType !== networkType) {
    const newRecord = {
      timestamp: Date.now(),
      ip: ip,
...
function fetchPublicNetworkInfo() {
  ...
  fetch(apiURL)
    ...
    .then((data) => {
      ...
      elLocation.textContent = city && country ? `${city}, ${country}` : country || 'N/A';
      ...
      elVPNDetect.textContent = vpnDetected ? 'Yes (Potential VPN/Proxy)' : 'No';
  ...
  return new Promise((resolve) => {
    ...
    pc.onicecandidate = (event) => {
      if (event.candidate && event.candidate.candidate) {
        ...
        const ipRegex = /([0-9]{1,3}(?:\.[0-9]{1,3}){3})|([0-9a-fA-F:]{4,})/;
...
function measurePing(callback) {
  ...
  pingImg.src = 'https://www.google.com/favicon.ico?cache=' + Math.random();
}
...
function measureDownloadSpeed(callback) {
  ...
  const imageUrl = 'https://upload.wikimedia.org/wikipedia/commons/5/5a/Big5-test.png';
